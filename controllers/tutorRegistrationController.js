  ...
  try {
    ...
    if (adminEmail) {
      ...
      const html = `
        ...
        <p><strong>Name:</strong> ${escapeHtml(tutor.name || "N/A")}</p>
        <p><strong>Subject:</strong> ${escapeHtml(tutor.interestedInSubjects || "N/A")}</p>
        <p><strong>Contact:</strong> ${escapeHtml(tutor.phone || tutor.email || "N/A")}</p>
        <p><strong>Email:</strong> ${escapeHtml(tutor.email || "N/A")}</p>
        <hr />
        <p><strong>Tutor Code:</strong> ${escapeHtml(tutor.tutorCode || "N/A")}</p>
        <p><strong>Gender/Age:</strong> ${escapeHtml(tutor.genderAge || "N/A")}</p>
        <p><strong>Teaching Type:</strong> ${escapeHtml(tutor.interestedInTeaching || "N/A")}</p>
        <p><strong>Address:</strong> ${escapeHtml(tutor.address || "N/A")}</p>
        <p><strong>Experience:</strong> ${escapeHtml(tutor.experienceOfTeaching || "N/A")}</p>
        <p><strong>Class:</strong> ${escapeHtml(tutor.interestedInClass || "N/A")}</p>
        <p><strong>Subjects:</strong> ${escapeHtml(tutor.interestedInSubjects || "N/A")}</p>
        <p><strong>Qualification:</strong> ${escapeHtml(tutor.qualification || "N/A")}</p>
        <p><strong>Fees Expectation:</strong> ${escapeHtml(tutor.feesExpectation || "N/A")}</p>
        <p><strong>Submitted At:</strong> ${tutor.createdAt.toLocaleString() || "N/A"}</p>
        
      `;
      void sendMail({ to: adminEmail, subject, html }).catch((mailErr) => {
        console.error(
          "Failed to send tutor registration email:",
          mailErr.message,
        );
      });
    }
...
exports.getTutorRegistration = async (req, res) => {
  try {
    ...
    if (!tutor)
      return res
        .status(404)
        .json({ success: false, message: "Tutor registration not found." });
...
exports.updateTutorRegistration = async (req, res) => {
  try {
    const tutor = await TutorRegistration.findByIdAndUpdate(
      req.params.id,
      req.body,
...
exports.getTutorStats = async (req, res) => {
  try {
    ...
    const pending = await TutorRegistration.countDocuments({
      status: "pending",
    });
    const approved = await TutorRegistration.countDocuments({
      status: "approved",
    });
